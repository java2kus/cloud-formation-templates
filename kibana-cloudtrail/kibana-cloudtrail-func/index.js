#!/usr/bin/env node
//
// Streams CloudTrail to Elasticsearch
//
            ...
            async.waterfall([
                    ...
                    function parseLogMessage(logMessage, next) {
                        ...
                        async.eachSeries(logMessage.logEvents, function(logEvent, callback2) {
                                ...
                                source = JSON.parse(logEvent.message);
                                ...
                                // geoip
                                // console.log('sourceIPAddress:', source.sourceIPAddress);
                                if (source.sourceIPAddress) {
                                    var geo = geoip.lookup(source.sourceIPAddress);
                                    // console.log('geo:', geo);
                                    if (geo) {
                                        // console.log('geo:', geo);
                                        source.geo = geo;
                                        source.geo.point = {};
                                        source.geo.point.lat = parseFloat(geo.ll[0]);
                                        source.geo.point.lon = parseFloat(geo.ll[1]);
                                    }
                                }
